...
test('renders Git Slice landing content and navigation', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /learn `gs fs` in three steps/i })).toBeVisible();
  await expect(page.getByText(/gs fs write \/\$USER\/demo\/hello\.txt --text "hello from gitslice"/i)).toBeVisible();
  await expect(page.getByText(/gs slice checkout <slice-id>/i)).toBeVisible();
});
