...
func printSliceHelp() {
	...
	fmt.Println("  checkout  Checkout a slice to working directory using its slice ID or slug (--files, --git for advanced local repo mode)")
	...
	fmt.Println("  sync      Sync the current checked out slice in place (follows checkout mode)")
	...
	fmt.Println("  diff      Show local changes for the current slice checkout")
	fmt.Println("  restore   Restore local files back to the recorded slice state")
