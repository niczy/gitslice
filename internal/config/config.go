...
type Config struct {
	...
	PostgresProjectionMaxConns int32

	// Merge-event projection workers build derived commit-history indexes.
	MergeEventProjectionEnabled      bool
	MergeEventProjectionWorkers      int
	MergeEventProjectionBatchSize    int
	MergeEventProjectionShardCount   int32
	MergeEventProjectionPollInterval time.Duration
...
func LoadConfig() (*Config, error) {
	...
	postgresProjectionMaxConns, err := getEnvOptionalInt32("POSTGRES_PROJECTION_MAX_CONNS")
	if err != nil {
		return nil, err
	}
	if strings.TrimSpace(os.Getenv("POSTGRES_PROJECTION_MAX_CONNS")) == "" {
		postgresProjectionMaxConns = 4
	}
	return &Config{
		...
		PostgresProjectionMaxConns: postgresProjectionMaxConns,
		MergeEventProjectionEnabled: getEnvBool(
			"MERGE_EVENT_PROJECTION_ENABLED",
			true,
		),
		MergeEventProjectionWorkers:      getEnvInt("MERGE_EVENT_PROJECTION_WORKERS", 1),
		MergeEventProjectionBatchSize:    getEnvInt("MERGE_EVENT_PROJECTION_BATCH_SIZE", 256),
		MergeEventProjectionShardCount:   int32(getEnvInt("MERGE_EVENT_PROJECTION_SHARDS", 1024)),
		MergeEventProjectionPollInterval: getEnvDuration("MERGE_EVENT_PROJECTION_POLL_INTERVAL", 250*time.Millisecond),
...
func (c *Config) Validate() error {
	...
	if c.PostgresProjectionMaxConns < 0 {
		return fmt.Errorf("POSTGRES_PROJECTION_MAX_CONNS must be >= 0")
	}
	if c.MergeEventProjectionWorkers < 0 {
		return fmt.Errorf("MERGE_EVENT_PROJECTION_WORKERS must be >= 0")
	}
	if c.MergeEventProjectionBatchSize < 0 {
		return fmt.Errorf("MERGE_EVENT_PROJECTION_BATCH_SIZE must be >= 0")
	}
	if c.MergeEventProjectionShardCount < 0 {
		return fmt.Errorf("MERGE_EVENT_PROJECTION_SHARDS must be >= 0")
	}
	if c.MergeEventProjectionPollInterval < 0 {
		return fmt.Errorf("MERGE_EVENT_PROJECTION_POLL_INTERVAL must be >= 0")
	}
	if c.MergeEventProjectionEnabled && c.PostgresProjectionMaxConns == 1 {
		return fmt.Errorf("MERGE_EVENT_PROJECTION_ENABLED requires POSTGRES_PROJECTION_MAX_CONNS to be 0/shared or at least 2")
	}
